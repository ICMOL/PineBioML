...
class feature_extension():
    ...
    def fit(self, x, y):
        ...
        self.monotonic_feature = x.columns[np.logical_or(x.min()>=0, x.max()<=0)]
        ...
        if self.pca:
            ...
            aev = 0
            counter = 0
            while aev < self.alpha:
                aev += evr[counter]
                counter += 1
                
            self.pca = PCA(counter)
            pcx = self.pca.fit_transform(x)
            pcx = pd.DataFrame(pcx, index = x.index, columns = [self.name + "pc"+str(i) for i in range(self.pca.n_components_)])
            #results.append(pcx)
        ...
        if self.pls:
            ...
            plsx = pd.DataFrame(plsx, index = x.index, columns = [self.name + "pls"+str(i) for i in range(4)])
            #results.append(plsx)
